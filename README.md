  ...
  - [Macros](#macros)
    ...
    - [Utils](#utils)
      ...
      - [Log Helpers](#log-helpers)
        ...
        - [`log_model_run_started_pre_hook(relation=this, message=None, load_interval_days=None)`](#log_model_run_started_pre_hookrelationthis-messagenone-load_interval_daysnone)
        - [`log_model_run_succeeded_post_hook(relation=this, message=None, load_interval_days=None)`](#log_model_run_succeeded_post_hookrelationthis-messagenone-load_interval_daysnone)
...
A BigQuery-specific materialization for incremental loads that includes logging of model run events. Supports partitioning and clustering configurations. Can be configured with `load_interval_days` to limit the maximum time window for loading data from the last successful run.
...
##### `apply_load_interval_limit(load_interval_days, window_start, window_end=run_started_at)`

Applies a load interval limit to the window_end timestamp. If load_interval_days is provided and greater than 0, and window_start is available, the window_end will be capped at window_start + load_interval_days to prevent excessive data loading.
...
##### `log_model_run_started_pre_hook(relation=this, message=None, load_interval_days=None)`

Pre-hook macro to log the start of a model run.

##### `log_model_run_succeeded_post_hook(relation=this, message=None, load_interval_days=None)`
